...
public class JdxzfuncChangeVideoModeToMov {
	 ...
	 public static void main(String[] args) {  
	    	...
	    	if(result == JFileChooser.APPROVE_OPTION)
	    	{
	    		...
	    		if (process()) {  
	           	 ...
	           	 jdxz.setText("爱你哟小辣鸡，大辣鸡帮你把视频转成mov格式了！");
	            ...
	            System.out.println("ok");  
	    			...
	    			JdxzDialog jdxz=new JdxzDialog();
	              	 jdxz.setBounds(300, 300, 400, 200);
	   				 jdxz.setVisible(true);
	   				 jdxz.getCancelButton().setVisible(false);
	              	 jdxz.setText("难受啊小辣鸡，大辣鸡不能帮你把视频转成mov格式了，呜呜呜！");
	              	 jdxz.getOkButton().addMouseListener(new MouseAdapter() {
	   					@Override
	   					public void mouseClicked(MouseEvent e) {
	   						jdxz.setVisible(false);
    ...
    private static boolean process() {  
        ...
        if (type == 0) {  
            ...
            status = processMov(inPATH);// 直接将文件转为flv文件  
        } else if (type == 1) {  
            String avifilepath = processAVI(type);  
            if (avifilepath == null)  
                return false;// avi文件没有得到  
            status = processMov(avifilepath);// 将avi转为flv  
        }  
        return status;  
    }  
    ...
    // ffmpeg能解析的格式：（asx，asf，mpg，wmv，3gp，mp4，mov，avi，flv等）  
    private static boolean processMov(String oldfilepath) {  
